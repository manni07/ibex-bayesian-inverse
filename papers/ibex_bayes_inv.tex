...
where limited samples of computer model evaluations ($n_M < 50$) are shown. For
...
$u$-value. Combine that with $n_M=66$ unique $u$-values used in runs available to
us and our response vector will have greater than one million elements. Even if
we used a smaller LHS space-filling design of size $n_M=15$ over the model
...
the Scaled Vecchia (SVEC) approximation introduced by \citet
...
by SVEC in the context of IBEX and its associated simulator. (We will circle
...
\caption{Conditioning sets $h(i)$ in a multivariate normal likelihood(Eq.~\ref
 {eq:vecchia}) for a single observation $Y_i^M$ using SVEC. The left panel
...
We begin evaluation of SVEC's performance on the IBEX simulator with
...
panels displayed with thin, solid borders). An SVEC GP surrogate is fit
...
experiment, we fit an SVEC GP surrogate on 65 of the $n=66$ model runs
...
back to the others in a moment. It is obvious that SVEC outperforms{\sf laGP}
...
to the target posterior. SEPIA performed similarly to SVEC, illustrating why
...
the two methods that are best equipped for large dimensions (SVEC and
...
used for SVEC and SEPIA, respectively.
...
SVEC and SEPIA is more nuanced. In the right panel of Figure
...
other contexts, such as a large number of simulation runs. SVEC does
...
performs better. Our preferred method, SVEC, shows consistency in
...
0.02$ (top right). Predicted output via a fitted SVEC surrogate at
...
take the estimated posterior mean and plug it in to our fitted SVEC
...
simulated observed satellite data, fit an SVEC surrogate on training
...
\ref{alg:gibbs} by fitting an SVEC GP surrogate to \textit{all} 66
...
\caption{Observed IBEX satellite data for years 2009-2011 (top left, top middle, bottom left). Predicted output via a fitted SVEC surrogate at
