...
To help fix ideas, consider the simpler example introduced in the
{\color{red}top row} of Figure \ref{f:toy_calib} {\color{red} (ignore
the bottom row for now)}. {\color{red}In the top left panel,} red stars indicate noisy
field measurements, $Y^F$, observed in replicate as $y_i
...
\begin{figure}[ht!]
\centering
\includegraphics[scale=0.51,trim=0 60 30 0,clip=TRUE]{logit1_obs.pdf}
\includegraphics[scale=0.51,trim=71 60 30 0,clip=TRUE]{logit1_est.pdf}
\includegraphics[scale=0.51,trim=31 60 25 0,clip=TRUE]{logit1_post_draws.pdf}
\includegraphics[scale=0.51,trim=0 0 30 45,clip=TRUE]{logit1_pois_obs.pdf}
\includegraphics[scale=0.51,trim=71 0 30 45,clip=TRUE]{logit1_pois_est.pdf}
\includegraphics[scale=0.51,trim=31 0 25 45,clip=TRUE]{logit1_pois_post_draws.pdf}
\caption{{\color{red}A simple inverse problem. Observations from the
field and a sample of computer model runs are displayed in the left column. On
the top, red stars are measurements with Gaussian noise. On the bottom, red
stars are draws from a Poisson that, when summed together, represent $Y^F$ in
Alg.~\ref{alg:gibbs}. Red red circles correspond to the quantity
$\frac{Y^F}{e(X^F)}$. The middle panels show computer model evaluations at
posterior draws and means. Right panels show posterior distributions of model
parameter $u$.}
\label{f:toy_calib}}
\end{figure}
...
from the posterior could proceed via Metropolis as in Algorithm \ref{alg:gibbs}.
...
in {\color{red}the top row of} Figure \ref{f:toy_calib} and align it more
closely with the IBEX data. Instead of observing a mean process $\mu(X^F)$
with random, Gaussian noise, suppose we are given counts, drawn from a Poisson
process with $\lambda(X^F) = m(u^\star, X^F)$, as shown in the
{\color{red}bottom} left panel of Figure \ref{f:toy_calib}. Computer model
runs $m(\cdot,\cdot) = \lambda(X)$ produce different mean surfaces given
unique values of $u = (u_1, u_2)$. Observed counts are shown as red stars. Red
circles indicate the sum of all counts at one input location (i.e. a single
row of $X^F$) divided by the sum of their corresponding exposure times
$e(\cdot)$. These points are the actual data we receive. Results from running
Algorithm \ref{alg:gibbs} are shown in the {\color{red}bottom} middle and
right panels of Figure \ref{f:toy_calib}. Similar to results in
{\color{red} the top row}, it is clear that the sampler searches a broad swath
of the model input space but hones in {\color{red}(perhaps more effectively
than before)} on the region where the true parameter value lies.
...
illustrate, recall Figure \ref{f:toy_calib} in
...
prohibitive cost. With the 1d example in Figure \ref{f:toy_calib}, we have
...
fastest methods {\color{red}on larger training/test sets} is shown in the
right panel, with different line types indicating neighborhood size ($m$) and
number of principal components ($n_k$) used for {\color{red}SVEC} and SEPIA,
respectively. {\color{red}Neither {\sf deepgp} or {\sf laGP} are shown for
...
\begin{figure}[ht!]
\centering
\includegraphics[scale=0.38,trim=5 15 100 45,clip=TRUE]{ibex_sim_field.pdf}
\includegraphics[scale=0.38,trim=50 15 100 45,clip=TRUE]{ibex_sim_est.pdf}
\includegraphics[scale=0.38,trim=50 15 15 45,clip=TRUE]{ibex_sim_mod.pdf}
\includegraphics[scale=0.38,trim=5 15 15 45,clip=TRUE]{ibex_post_est.pdf}
\includegraphics[scale=0.38,trim=5 15 0 45,clip=TRUE]{ibex_sim_resids.pdf}
\caption{Estimate of ENA rate
(counts/seconds) based on synthetic Poisson draws from {\color{red} an
``unknown''} computer model setting and varying exposure times (top left).
IBEX simulator output of ENA rate at \textit{parallel mean free path} $= 1750$
and \textit{ratio} $= 0.02$ (top middle). Predicted output via a fitted
{\color{red}SVEC} surrogate at estimated calibration parameters (top right).
{\color{red}Residuals between predicted and simulated output (bottom right)}. Posterior
samples of model parameter $u$ as a kernel density (bottom left).
...
concentrate mass around the true, {\color{red}``unknown''} model parameter. As
a sanity check, we take the estimated posterior mean and plug it in to our
fitted {\color{red}SVEC} GP surrogate of the IBEX simulator. The predicted
output is shown in the {\color{red}top right} panel of Figure
\ref{f:ibex_synth_ex}. There are some nearly imperceptible differences between
the estimated sky map and the true $\lambda^\star$ (i.e., the shape and
intensity of the ribbon vary slightly between the two). {\color{red}These
residuals between simulated and predicted output (bottom right panel) are
orders of magnitude smaller than the response, but do exhibit structure, which
we will address in Section \ref{sec:discuss}}. However, from a purely visual
standpoint, it appears that each map could have reasonably generated the data
we ``observe'' in the top left panel.
...
\ref{f:ibex_real_ex}, but even more extreme, as the Metropolis sampler pushes up
